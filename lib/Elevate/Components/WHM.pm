...
sub _blocker_cpanel_needs_update ( $self, $major ) {
    if ( !$self->getopt('skip-cpanel-version-check') ) {
        my $tiers_obj        = Cpanel::Update::Tiers->new( logger => Log::Log4perl->get_logger(__PACKAGE__) );
        my $expected_version = $tiers_obj->get_flattened_hash()->{"11.$major"} // "";                            # $major could be a dev version and thus odd and without a tier, so give a defined value in that case
        if ( !length $expected_version || Cpanel::Version::Compare::compare( $Cpanel::Version::Tiny::VERSION_BUILD, '<', $expected_version ) ) {
